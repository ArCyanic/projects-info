...
import stud.queue.ZobristFrontier;
...
public abstract class EngineFeeder {
     ...
     */
    public Frontier getListFrontier(EvaluationType type) {
        return new ListFrontier(Node.evaluator(type));
    }

    public Frontier getPriorityFrontier(EvaluationType type){
    ...
    public final Searcher getAStar(HeuristicType type) {
        ...
        Frontier frontier = getListFrontier(EvaluationType.FULL);
//        frontier = getPriorityFrontier(EvaluationType.FULL);
        // 以HashSet作为Explored表
    ...
    public final Searcher getDijkstra() {
        ...
        Frontier frontier = getListFrontier(EvaluationType.PATH_COST);
        // 以HashSet作为Explored表
        Set<State> explored = new HashSet<>();
